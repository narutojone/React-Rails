...
class InnerPageLayout extends Component {
  ...
  startGroupCall = (appId, appName, appAvatar) => {
  ...
  initPusher = () => {
    const userInit = window.localStorage.getItem('mapp_token') || window.localStorage.getItem('mapp_client_id');
    if (!userInit) {
      return;
    }
    ...
    const authToken = window.localStorage ? window.localStorage.getItem('mapp_token') : '';
    const clientID = window.localStorage ? window.localStorage.getItem('mapp_client_id') : '';
    if (!authToken) {
  ...
  render() {
    ...
    return (
      <div id='myapp-root-app' className='app-container'>
        <Helmet
        ...
        <Snackbar
          ...
          message='MyApp has been updated to a new version.'
          action='Refresh'
