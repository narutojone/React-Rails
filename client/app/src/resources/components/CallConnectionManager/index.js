...
class CallConnectionManager extends Component {
  ...
  startGroupCall = (channelId, appName, appAvatar) => {
    // If there is current open or requesting call, cannot start another
    const { currentCall, outgoingCall } = this.state;
    if (currentCall || outgoingCall) {
      return;
          ...
          onConnected: (session, publisher) => {
            ...
            this.setState({
              ...
              currentCall: {
                ...
                app_name: outgoingCall.app_name,
                app_avatar: outgoingCall.app_avatar,
                callStartTime: Math.floor(Date.now() / 1000),
                callStartCountingTimer: setInterval(this.callTimeCount, 1000),
              },
              callTimeHours: 0,
  ...
  render() {
    ...
    const avatar = currentCall ?
      (currentCall.group_call ? currentCall.app_avatar : currentCall.user_avatar)
      :
      outgoingCall && (outgoingCall.group_call ? outgoingCall.app_avatar : outgoingCall.user_avatar);
    const title = currentCall ?
      (currentCall.group_call ? currentCall.app_name : currentCall.username)
      :
      outgoingCall && (outgoingCall.group_call ? outgoingCall.app_name : outgoingCall.username);
