...
import IconApp from 'material-ui/svg-icons/action/donut-small';
...
import NearYouApp from '@dashboard/components/NearYouApp';
...
class DashboardContainer extends Component {
  ...
  handleViewMoreInterestingPeoples = (viewMore) => {
    const { viewMoreTrendingApp } = this.state;
    this.setState({
      viewMoreInterestingPeoples: viewMore,
      viewMoreTrendingApp: !viewMore && viewMoreTrendingApp,
    });
  }

  handleViewMoreTrendingApp = (viewMore) => {
    const { viewMoreInterestingPeoples } = this.state;
    this.setState({
      viewMoreInterestingPeoples: !viewMore && viewMoreInterestingPeoples,
      viewMoreTrendingApp: viewMore,
    });
  }
  ...
  setNewImg = (img, cropData) => {
    ...
    .then((graphQLResult) => {
      ...
      if (errors) {
        ...
        const newUser = JSON.parse(localStorage.getItem('mapp_user'));
        newUser.avatar_url = data.changeUserAvatar.user.avatar_url;
        localStorage.setItem('mapp_user', JSON.stringify(newUser));
        this.setState({
          avatarChanged: false,
          showAvatarEditor: false,
          files: [],
  ...
  handleDialogCreateAppOpen = () => {
    this.setState({
      openCreateApp: true,
    });
  };

  handleDialogCreateAppClose = (path) => {
  ...
  setNewCover = (img) => {
    const user = this.state.user && this.state.user.id ? this.state.user : JSON.parse(localStorage.getItem('mapp_user'));
  ...
  saveUserDescription = () => {
    const user = JSON.parse(localStorage.getItem('mapp_user'));
  ...
  render() {
    ...
    if (localStorage.getItem('mapp_activeCache')) {
      activeCache = JSON.parse(localStorage.getItem('mapp_activeCache'));
    }
    ...
    const sidebarContent = <div>
      <div className='myb-feed-types fixed-types'>
        ...
        <Link
          className={this.props.location.pathname === '/app-near-you' ? 'myb-feed active' : 'myb-feed'}
          to={this.props.location.pathname === '/app-near-you' ? null : '/app-near-you'}
        >
          <IconNearMe style={CommonStyles.iconStyle} /><span className='myb-feed-label'>App near you</span>
        </Link>
        <Link
        ...
        <Link
          className={this.props.location.pathname === '/myapp' ? 'myb-feed active' : 'myb-feed'}
          to={this.props.location.pathname === '/myapp' ? null : '/myapp'}
        >
          <IconApp style={CommonStyles.iconStyle} /><span className='myb-feed-label'>My App</span>
        </Link>
        <Link
        ...
        {$(window).width() < 480 ?
          <Link className='myb-feed' onClick={this.handleDialogCreateAppOpen}>
            <IconContentAdd style={CommonStyles.iconStyle} />
            <span className='myb-feed-label'>Add app</span>
          </Link>
          :
          null
        }
    ...
    const { openMessageFriendId, viewMoreInterestingPeoples, viewMoreTrendingApp } = this.state;
    ...
    else if (this.props.params.dash_tab === 'app-near-you') {
      showDashTab = <NearYouApp />;
    }
    ...
    else if (this.props.params.dash_tab === 'myapp') {
      showDashTab = <MyApp />;
    }
    ...
    if (this.props.getMyApp.my_app) {
      userApp = this.props.getMyApp.my_app.edges.map((app, index) => {
        let truncatedString = app.node.name;
        ...
        return <span className='stat-item' key={index}>
          <div className='app-wrapper'>
            <Link to={`/app/${app.node.permalink}`}>
              <img src={app.node.avatar_url} />
            </Link>
            <div className='app-info'>
              <Link className='app-name' to={`/app/${app.node.permalink}`}>
                {truncatedString}
              </Link>
              <div className='app-description'>
                {app.node.description}
              </div>
            </div>
          </div>
        </span>;
    ...
    const myPages = this.props.params.dash_tab === 'myapp'
      || this.props.params.dash_tab === 'myfriends'
      || this.props.params.dash_tab === 'app-near-you'
      ...
      <div className='myb-dashboard-row' style={{ margin: isWidePage ? '0' : '0 -15px' }}>
        ...
        <div className='myb-dashboard-center-without-right' style={{ padding: isWidePage ? '0 16px 0 0' : '40px 0' }}>
                    ...
                    <div className="user-main-stat-info">
                      ...
                      <div className="user-stat-block">
                        <Link
                          ...
                          className='view-all'
                        >
                          <span className="digit">{userApp.length}</span> App
                        </Link>
                        <Link
                          to='/myfriends'
                          className='view-all'
          ...
          <Dialog
            title='Create App'
            modal={false}
            open={this.state.openCreateApp}
            onRequestClose={this.handleDialogCreateAppClose}
          ...
          >
            <CreateOrUpdateApp closeDialog={this.handleDialogCreateAppClose} refetchApp={this.refetchApp} />
          </Dialog>
          {showDashTab}
        </div>
        {/* !this.props.params.dash_tab && !this.props.params.activity_id ?
          <div className='stat-block'>
            <div className='user-stat-row'>
              ...
              <div className='user-app user-column'>
                <div className='user-header'>
                  {userApp.length} App
                  {userApp.length > 0 ?
                    <Link
                ...
                <div className='content-wrapper'>
                  {userApp.length > 0 ? userApp : 'App not found'}
                </div>
              </div>
