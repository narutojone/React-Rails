...
class Notifications extends Component {
  ...
  listenPusher = () => {
    ...
    channel.bind('notification_added', (data) => {
      const notifications = this.state.notifications ? this.state.notifications : this.props.data.myNotifications.edges;
  ...
  loadMoreNotifications = () => {
    let notifications = this.state.notifications ? this.state.notifications : this.props.data.myNotifications.edges;
    ...
    this.props.client.query({
      query: gql`
        query getMyNotifs($lastCursor: String!) {
          myNotifications(first: 10, after: $lastCursor) {
            edges {
              cursor
              node {
                code
      ...
      if (errors) {
        ...
        if (data.myNotifications && data.myNotifications.edges.length > 0) {
          notifications = notifications.concat(data.myNotifications.edges);
        }
      }
  ...
  render() {
    ...
    if (!this.props.data.myNotifications) {
      return (
        <div>
      ...
      const notifsArray = this.state.notifications ? this.state.notifications : this.props.data.myNotifications.edges;
...
export default withApollo(hoc(Notifications));
