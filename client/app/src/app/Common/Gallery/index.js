...
class Gallery extends Component {
  ...
  subscribeToPusherChannel = () => {
    ...
    const appPermalink = this.props.params.permalink;

    if (appPermalink) {
      let appChannel = pusher.channels.channels[`private-app-${appPermalink}`];
      if (!appChannel) {
        appChannel = pusher.subscribe(`private-app-${appPermalink}`);
      }
      appChannel.bind('album_media_count_changed', this.handleAlbumCounters);
    }
    else {
  ...
  unsubscribeFromPusherChannel = () => {
    ...
    if (appPermalink) {
      const appChannel = pusher.channels.channels[`private-app-${appPermalink}`];
      if (!appChannel) {
        appChannel.unbind('album_media_count_changed', this.handleAlbumCounters);
        pusher.unsubscribe(`private-app-${appPermalink}`);
      }
    }
    else {
      const user = JSON.parse(localStorage.getItem('mapp_user'));
  ...
  submitForm = (data) => {
    ...
    if (this.props.permalink) {
      params = '$app_id: Int!, $albumName: String!, $albumDescription: String!, $privacy: String!';
      input = 'app_id: $app_id, name: $albumName, description: $albumDescription, privacy: $privacy';
      vars = {
  ...
  render() {
    ...
    if (this.props.data.loading) {
      ...
      if (this.props.username) {
        ...
        galleryLink = '/app/' + this.props.permalink + '/gallery';
      }
