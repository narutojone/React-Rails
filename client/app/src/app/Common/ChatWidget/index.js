...
class ChatWidget extends Component {
  ...
  onMessageCreated = (data) => {
    ...
    const username = JSON.parse(localStorage.getItem('mapp_user')).username;
  ...
  startGroupCall = () => {
    ...
    if (activeChannelId) {
      ...
      callFunctions.startGroupCall(activeChannelId, app.name, app.avatar_url);
    }
  }
  ...
  render() {
    ...
    const { app, chatWidgetId } = this.props;
    const username = JSON.parse(localStorage.getItem('mapp_user')).username;
    const { chat_widget, appMembers } = this.props.data;
    const {
    ...
    return (
      <div className='myb-messages-box chatwidget' style={{ height: messagesContainerHeight }}>
        <div className='topbar'>
          ...
          {
            activeChannelId > 0 && app.user_role !== 'guest' && <div style={{ padding: '15px 20px 0', display: 'inline-block',
            float: 'right',
            marginTop: -10, }}>
              <RaisedButton
                primary
        ...
        <div className={activeChannelId > 0 ? 'channels-area selected' : 'channels-area'} ref='channels'>
          {
            app.user_role !== 'guest' && <div style={{ padding: '15px 20px 0' }}>
              <RaisedButton
                primary
                label='Create Channel'
                style={callButtonStyle}
        ...
        <div className={activeChannelId > 0 ? 'chat-area selected' : 'chat-area'} ref='chatbox'>
          <div style={chatWidgetTopbarStyle}>
            ...
            <div style={callButtonContainerStyle}>
              {
                activeChannel.node && app.user_role !== 'guest' && (
