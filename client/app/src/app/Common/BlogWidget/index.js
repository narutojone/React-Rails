...
class BlogWidget extends Component {
  ...
  listenPusher = () => {
    ...
    let appChannel = pusher.channels.channels['private-app-' + this.props.appName];
    if (!appChannel) {
      appChannel = pusher.subscribe('private-app-' + this.props.appName);
    }
    appChannel.bind('feed_item_added', this.onFeedItemAdded);
    appChannel.bind('post_removed', this.onFeedItemRemoved);
  }
  ...
  unlistenPusher = () => {
    ...
    const appChannel = pusher.channels.channels['private-app-' + this.props.appName];
    if (appChannel) {
      appChannel.unbind('feed_item_added', this.onFeedItemAdded);
      appChannel.unbind('post_removed', this.onFeedItemRemoved);
    }
  }
