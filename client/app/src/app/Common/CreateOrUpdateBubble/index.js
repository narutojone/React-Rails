...
class CreateOrUpdateApp extends Component {
  constructor(props) {
    super(props);
  ...
  submitForm = (data) => {
    ...
    if (this.props.update) {
      params.id = parseInt(this.props.app.id);
      mutation = this.props.updateApp;
    }
    else {
      params.widgets = widgets;
      params.appType = appType;
      mutation = this.props.createNewApp;
    }
    ...
    .then((graphQLResult) => {
      ...
      if (errors) {
        ...
        if (self.props.update) {
          notify.show('App updated successfully', 'success');
          ReactGA.event({
            category: 'App',
            action: 'Updated a app',
          });
          self.props.closeDialog();
          if (self.props.refetchApp) {
            self.props.refetchApp();
          }
        }
        else {
          notify.show('App created successfully', 'success');
          ReactGA.event({
            category: 'App',
            action: 'Created a app',
          });
          self.props.closeDialog(`/app/${data.createApp.app.permalink}`);
        }
      }
    }).catch((error) => {
  ...
  setInitialState(props) {
    if (props.update && props.app) {

      const interests = [];
      if (props.app.interests) {
        props.app.interests.edges.map(interest => {
          interests.push({
      ...
      const widgets = [];
      if (props.app.events_widget_id) {
        widgets.push({
          key: widgets.length + 1,
          label: 'Events',
        });
      }
      if (props.app.blog_widget_id) {
        widgets.push({
          key: widgets.length + 1,
          label: 'Blog',
        });
      }
      if (props.app.chat_widget_id) {
        widgets.push({
          key: widgets.length + 1,
          label: 'Chat',
        });
      }
      if (props.app.gallery_widget_id) {
        widgets.push({
          key: widgets.length + 1,
          label: 'Gallery',
        });
      ...
      this.setState({
        interests,
        widgets,
        app_type: (props.app.kind === 'privy' ? 'private' : 'public'),
      });
    }
  }
  ...
  render() {
    ...
    const { update, app } = this.props;

    return (
      <div className='complete-profile-wrapper create-app'>
        <FormsyForm
          ...
          <Paper style={paperStyle} zDepth={0} rounded={false}>
            {
              ...
              <FormsyRadioGroup
                name='app_type'
                defaultSelected={this.state.app_type}
                style={{ margin: '8px 0' }}
                onChange={(event, value) => this.setState({ app_type: value })}>
                <FormsyRadio
                  value='private'
                  label='Private'
            ...
            <FormText
              name='appName'
              defaultValue={update && app ? app.name : ''}
              ...
              required
              requiredError='You must enter app name'
              ignorePristine={ignorePristine}
            />
            <FormText
              name='description'
              defaultValue={update && app ? app.description : ''}
              floatingLabelText='App description'
              underlineShow={false}
              inputStyle={inputStyle}
              floatingLabelStyle={labelStyle}
              floatingLabelFocusStyle={labelFocusStyle}
            ...
            <FormText
              name='zip_code'
              defaultValue={update && app ? app.zip_code : ''}
            ...
            {this.state.app_type === 'private' ?
              ...
              <div>
                <AutoComplete
                  ...
                  floatingLabelText='Enter App Keywords'
            ...
            <RaisedButton
              ...
              :
                (this.props.update ? 'Update App' : 'Create App')
              }
              labelColor='#FFFFFF'
...
export default hoc(CreateOrUpdateApp);
