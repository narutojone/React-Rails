...
class Search extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tempDataSource: [],
      keywords: [],
      zip_code: '', // JSON.parse(localStorage.getItem('mapp_user')).zip_code,//this.props.searchKeyword ? JSON.parse(localStorage.getItem('mapp_user')).zip_code : '',
      radius: 0,
      users: null,
      app: null,
      open: false,
  ...
  handleUpdateCustomKeywords = (value) => {
    ...
    this.searchApp(value, zip_code, radius, true);
  };
  ...
  handleDeleteKeyword = (key) => {
    ...
    if (keywords.length === 0) {
      ...
      this.searchUsersApp(keywords, false);
    }
  };
  ...
  searchApp = (keywords, zip_code, radius, suggestion) => {
    const self = this;
    this.setState({
      suggestSearch: suggestion,
    });
    let query = gql`
      query searchApp($first: Int!, $keywords: [String]!, $zip_code: String!, $radius: Int!) {
        findApp(first: $first, keywords: $keywords, zip_code: $zip_code, radius: $radius) {
          edges {
            node {
              permalink
              avatar_url(version: "micro")
    ...
    /*if (suggestion) {
      query = gql`
        query suggestApp($first: Int!, $keyword: String!) {
          suggestApp(first: $first, keyword: $keyword) {
            edges {
              node {
                permalink
                avatar_url(version: "micro")
    ...
    this.props.client.query({
      ...
      if (errors) {
        ...
        this.setState({
          app: data.suggestApp ? data.suggestApp.edges : data.findApp.edges,
          stopSearch: true,
          open: true,
        });
      }
  ...
  searchUsersApp = (keys, suggestion = false) => {
    let keywords = [];
    if (keys) {
    ...
    this.searchApp(keywords, zip_code, radius, suggestion);
  }
  ...
  toggleSearch = (value) => {
    ...
    setTimeout(() => this.searchUsersApp(), 500);
  }
  ...
  render() {
    ...
    if (this.state.app) {
      ...
      if (!app.length) {
        resultSearchApp = (<List>
          <Subheader style={subheaderStyles}>App</Subheader>
          <ListItem style={{ fontSize: 13 }} className='search-empty-item' primaryText='No app found' />
        </List>);
      }
      else {
        resultSearchApp = (<List>
          <Subheader style={subheaderStyles}>App<Link className='search-more-link' to={`/search/app?q=${this.state.searchText}`}>More</Link></Subheader>
          {app.map((app, index) => (
            <SearchItem key={index} type='app' node={app.node} quickSearch goToPage={this.goToPage} />
          ))}
        </List>);
      }
    }
