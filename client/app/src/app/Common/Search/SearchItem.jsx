  ...
  render() {
    ...
    if (this.props.quickSearch) {
      if (this.props.type === 'user') {
        ...
        const title = <Link to={`/app/${searchNode.permalink}`} style={titleStyle}>
          {searchNode.name}
        </Link>
        results = <ListItem
          leftAvatar={<Avatar size={32} src={searchNode.avatar_url} />}
          primaryText={title}
          innerDivStyle={{padding: '16px 64px 16px'}}
          onClick={() => this.props.goToPage(`/app/${searchNode.permalink}`)}
        />
      }
    } else {
      if (this.props.type === 'user') {
        ...
        results = (<div className="search-result-item myb-message">
          <div className="search-result-item-left">
            <Link to={`/app/${searchNode.permalink}`} className="user-avatar feed-user-avatar">
              <img src={searchNode.avatar_url} role="presentation" />
            </Link>
            <div className="search-result-item-content">
              <Link to={`/app/${searchNode.permalink}`} style={titleStyle}>
                {searchNode.name}
              </Link>
              <div>
          ...
          <div>
            <Link to={`/app/${searchNode.permalink}`}>
              <FlatButton
                primary
                className="search-item-view"
                label="View"
