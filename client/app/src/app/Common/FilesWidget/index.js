...
class FilesWidget extends Component {
  ...
  subscribeToPusher = () => {
    ...
    const channelName = `private-app-${app.permalink}`;
    let appChannel = pusher.channels.channels[channelName];
    if (!appChannel) {
      appChannel = pusher.subscribe(channelName);
    }
    appChannel.bind('file_uploaded', this.handleFileUploaded);
    appChannel.bind('file_deleted', this.handleFileDeleted);
    appChannel.bind('document_created', this.handleDocumentCreated);
    appChannel.bind('document_updated', this.handleDocumentUpdated);
    appChannel.bind('document_deleted', this.handleDocumentDeleted);
  }
  ...
  unsubscribeFromPusher = () => {
    ...
    const pusher = this.context.pusher;
    const channelName = `private-app-${app.permalink}`;
    const appChannel = pusher.channels.channels[channelName];
    if (appChannel) {
      appChannel.unbind('file_uploaded', this.handleFileUploaded);
      appChannel.unbind('file_deleted', this.handleFileDeleted);
      appChannel.unbind('document_created', this.handleDocumentCreated);
      appChannel.unbind('document_updated', this.handleDocumentUpdated);
      appChannel.unbind('document_deleted', this.handleDocumentDeleted);
    }
  }
  ...
  getData = () => {
    ...
    this.props.client.query({
      query: gql`
        query uploadedFilesQuery($app_id: Int!) {
          appMembers(app_id: $app_id) {
            edges {
              cursor
              node {
                id
      ...
      if (errors) {
        ...
        const { appMembers, uploadedFiles, sharedDocuments } = data;
        this.setState({
          dataLoaded: true,
          files: uploadedFiles.edges,
          documents: sharedDocuments.edges,
          members: appMembers.edges,
        });
      }
    }).catch((error) => {
  ...
  deleteDocument = (documentId) => {
    ...
    if (i >= 0) {
      if (app.user_role === 'guest' && parseInt(documents[i].node.owner_id) !== parseInt(user.id)) {
  ...
  uploadFile = (file, callbacks) => {
    ...
    req.set('Authorization', localStorage.getItem('mapp_token'));
    req.set('Client-ID', localStorage.getItem('mapp_client_id'));
    req.field('app_id', app.id);
