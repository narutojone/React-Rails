...
class NotificationItem extends Component {
  ...
  onClickNotification = (notification, index, notifType, unread) => {
    ...
    let pageUrl = '';
    if (!notification.comment_id && notification.app) {
      pageUrl = `/app/${notification.app.permalink}`;
    }
  ...
  acceptInvitation = (notification, index) => {
    ...
    .then((graphQLResult) => {
      ...
      if (errors) {
        ...
        ReactGA.event({
          category: 'App',
          action: 'Accepted an invitation to app',
        });
        setTimeout(() => self.props.refetchNotifs(), 1000);
        setTimeout(() => self.props.router.push(`/app/${notification.app.permalink}`), 1500);
      }
    }).catch((error) => {
      notify.show(error.message, 'error');
      self.props.refetchNotifs();
  ...
  declineInvitation = (notification, inde) => {
    ...
    .then((graphQLResult) => {
      ...
      if (errors) {
        ...
        ReactGA.event({
          category: 'App',
          action: 'Declined an invitation to app',
        });
        setTimeout(() => self.props.refetchNotifs(), 1000);
      }
    }).catch((error) => {
  ...
  render() {
    ...
    if (notif.text === 'wants to add you as a friend') {
    ...
    else if (notif.text === 'accepted your invitation to join app' || notif.text === 'declined your invitation to join app') {
    }
    else if (notif.text === 'You were invited to join the app') {
      actionButtons = (<div className='action-buttons'>
        <FlatButton
          style={styleAcceptButton}
          hoverColor='#52b97e'
      ...
      const type_app_msg = notif.app.type === 'privy' ? 'the private app' : 'the public app';
      message = (<span>
        {notif.text.replace('the app', type_app_msg)}&nbsp;
        <Link to={`/app/${notif.app.permalink}`}>
          {notif.app.name}
        </Link>
      </span>);
    }
    ...
    if (
        notif.text.indexOf('left your app') > -1 ||
        notif.text.indexOf('accepted your invitation to join app') > -1 ||
        notif.text.indexOf('declined your invitation to join app') > -1 ||
      ...
      if (notif.app) {
        message = (<span>
          ...
          <Link
            className='app-link'
            onClick={() => this.onClickNotification(`/app/${notif.app.permalink}`, this.props.index, 'stop_page', notif.unread)}
            to={`/app/${notif.app.permalink}`}
          >
            {notif.app.name}
          </Link>
        </span>);
    ...
    if (notif.text.indexOf('Your Video / Image was commented') > -1) {
      if (notif.app) {
        message = (<span>
          ...
          <Link
            className='app-link'
            onClick={() => this.onClickNotification(`/app/${notif.app.permalink}/gallery`, this.props.index, 'stop_page', notif.unread)}
            to={`/app/${notif.app.permalink}/gallery`}
          >
            {notif.app.name}
          </Link>
          &nbsp;app.
