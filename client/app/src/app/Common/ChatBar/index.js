...
import AppMenu from '@dashboard/components/AppMenu';
...
class ChatBar extends Component {
  ...
  toggleAppSearch = (search) => {
    this.setState({
      showAppSearch: search,
    });
  }
  ...
  listenPusher = () => {
    ...
    channelD.bind('total_unread_items_count_changed', (data) => {
      const _appCounters = self.state.appCounters;
      const appCounters = { ..._appCounters };
    ...
    channel.bind('user_joined_app', this.handleUserJoinApp);
    channel.bind('user_left_app', this.handleUserLeftApp);
  }
  ...
  unlistenPusher = () => {
    ...
    channel.unbind('user_joined_app');
    channel.unbind('user_left_app');
  ...
  handleUserJoinApp = (data) => {
    this.setState({
      refetchApp: true,
    });
  }

  handleUserLeftApp = (data) => {
    this.setState({
      refetchApp: true,
    });
  }
  ...
  checkForInvitingFBFriends = () => {
    ...
    if (code) {
      ...
      const token = window.localStorage.getItem('mapp_token');
      const clientID = window.localStorage.getItem('mapp_client_id');
      ...
      request.end((err, res) => {
        if (err) {
          ...
          if (res.body.access_token) {
            const message = 'Try%20MyApp!%20You%27ll%20like%20it!';
  ...
  render() {
    ...
    const { searchOpen, keyword, keywordSearch, allApp, fbDialogOpen, fbRequestUrl } = this.state;
    const { currentUser } = this.props.data;
          ...
          <div className='myb-chatbar-content'>
            ...
            <div className='chatbar-users'>
              ...
              {friends.length > 0 ?
                ...
                :
                <div className='no-app'>You dont have friends yet</div>
              }
              </Scrollbars>
            ...
            <div className='myb-feed-types app'>
              ...
              <a className='myb-feed' style={{ paddingTop: 0 }} onClick={this.handleDialogCreateAppOpen}>
                <IconContentAdd style={CommonStyles.dashAppMenu.addAppIconStyle} />
                <span className='myb-feed-label' style={{ marginLeft: '14px' }}>Add app</span>
              </a>
              ...
              >
                <CreateOrUpdateApp
                  closeDialog={this.handleDialogCreateAppClose}
                  refetchApp={this.refetchApp}
                />
              </Dialog>

              {/* this.state.showAppSearch || this.state.keywordSearch !== '' ?
                <SearchApp onSearch={(newKeyword) => {
                  if (keywordSearch !== newKeyword) {
                    this.setState({ keywordSearch: newKeyword, allApp: false });
                  }
                }}
                />
                :
              ...
              */}
              ...
              >
                <AppMenu
                  ...
                  allApp={allApp}
                  appCounters={this.state.appCounters}
                  toggleAppSearch={this.toggleAppSearch}
                  refetchApp={this.state.refetchApp}
                  stopRefetchApp={this.stopRefetchApp}
                />
              </Scrollbars>
            </div>
