class Api::V1::Widgets::GalleryWidget::MediaController < ApplicationController
  ...
  def create
    app = @gallery.app
    bm = current_user.app_members.where(app_id: app.id).first
    if app.nil? or bm.nil?
      render 'api/v1/shared/failure', locals: {errors: [{message: 'You are not a member of this app'}]}, status: :unauthorized
    else
      ...
      if @media.save
        ...
        Activity.create(name: "galleries.create_media", user_id: current_user.id, user_ip: current_user.current_sign_in_ip,
                        ...
                        app_id: app.id, extra_data: extra_data)
  ...
  def update
    if current_user == @media.uploader or current_user.can_manage?(@gallery.app)
      if params[:picture_file].present?
      ...
      if @media.update_attributes(media_params)
        privacy = @gallery.app.privy? ? Activity.privacies[:p_private] : Activity.privacies[:p_public]
        Activity.create(name: "galleries.update_media", user_id: current_user.id, user_ip: current_user.current_sign_in_ip,
                        object_id: @media.id, object_type: @media.class.name, privacy: privacy, feed: true,
                        app_id: @gallery.app.id, extra_data: {title: @media.title, old_title: old_title})
  ...
  def destroy
    if current_user == @media.uploader or current_user.can_manage?(@media.app)
      @media.destroy
      if @media.destroyed?
        privacy = @media.app.privy? ? Activity.privacies[:p_private] : Activity.privacies[:p_public]
