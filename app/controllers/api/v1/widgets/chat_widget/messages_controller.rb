class Api::V1::Widgets::ChatWidget::MessagesController < ApplicationController
  ...
  before_action :find_app, except: [:remove_attachment]
  ...
  def create
    ...
    if @chat_message.save
      ...
      @message[:channel_name] = @app.permalink
  ...
  def find_app
    @app = @chat.widget.app rescue nil
    if @app.nil?
      render 'api/v1/shared/failure', locals: {errors: [{message: "App not found"}]}, status: :not_found
    end
  end

  def allow_only_members
    if AppMember.where(user_id: current_user.id, app_id: @app.id).first.nil?
      render 'api/v1/shared/failure', locals: { errors: [{ message: 'access denied' }] }, status: :unauthorized
      return
    end
  end
