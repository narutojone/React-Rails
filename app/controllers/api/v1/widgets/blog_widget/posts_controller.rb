class Api::V1::Widgets::BlogWidget::PostsController < ApplicationController
  ...
  def update
    ...
    if app.nil? or bm.nil?
      render 'api/v1/shared/failure', locals: {errors: [{message: 'You are not a member of this app'}]}, status: :unauthorized
    elsif app.privy? and bm.guest?
      render 'api/v1/shared/failure', locals: {errors: [{message: 'You can\'t manage this app'}]}, status: :unauthorized
    else
    ...
    if app.nil? or bm.nil?
      ...
      if @post.destroyed?
        ...
        Activity.create(name: "blogs.destroy_post", user_id: current_user.id, user_ip: current_user.current_sign_in_ip,
                        privacy: privacy, feed: false, app_id: app.id, extra_data: {title: @post.title})
