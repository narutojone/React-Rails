class Api::V1::MediaController < ApplicationController
  ...
  def create
    ...
    if params[:app_id].present?
      ...
      if app.nil? or app.gallery_widget.nil?
        render 'api/v1/shared/failure', locals: {errors: [{message: 'app or gallery is not found'}]}, status: :unprocessable_entity and return
      else
        @media.mediable = app.gallery_widget
      end
    end
    ...
    if @media.save
      ...
      app = nil
      if @media.try(:mediable).try(:app).present?
        activity_name = 'galleries.create_media'
        app = @media.mediable.app
        privacy = app.privy? ? Activity.privacies[:p_friends] : Activity.privacies[:p_public]
        extra_data.merge!(app: {name: app.name})
      ...
      Activity.create(name: activity_name, user_id: current_user.id, user_ip: current_user.current_sign_in_ip,
                      ...
                      app_id: app.try(:id), extra_data: extra_data)
  ...
  def destroy
    unless @media.uploader == current_user
      app = @media.gallery.try(:app)
      if app.nil? or !current_user.can_manage?(app)
    ...
    if @media.destroyed?
      ...
      Activity.create(name: activity_name, user_id: current_user.id, user_ip: current_user.current_sign_in_ip,
                      privacy: privacy, feed: false, app_id: app.try(:id), extra_data: extra_data)
    end
  ...
  def put
    if params[:album_id].blank?
      ...
      if @album.nil?
        ...
        app = @media.gallery.try(:app)
        if @media.uploader == current_user || app.present? && current_user.is_member_of?(app)
  ...
  def upload_media(type)
    ...
    if media_count == 1
      ...
      if app.present?
        activity_name = 'galleries.create_media'
        privacy = app.privy? ? Activity.privacies[:p_private] : Activity.privacies[:p_public]
        extra_data.merge!(app: {name: app.name})
      elsif @album.present?
        activity_name = 'albums.create_media'
        privacy = Activity.privacies[:p_private]
  ...
  def check_gallery
    if params[:gallery_id].present?
      ...
      elsif !current_user.is_member_of?(@gallery.app)
  ...
  def notify!
    if @album.present?
      if @gallery.nil?
        ...
        app_data = { id: app.id, permalink: app.permalink }
      end
